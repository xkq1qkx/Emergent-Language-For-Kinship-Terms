...
class DSpritesDataset(Dataset):
    def __init__(self, path_to_data, subsample=100, transform=None, image=False):
        ...
        self.labels = np.load(path_to_data, encoding="bytes")["latents_classes"][
            ::subsample
        ]
        self.latent_values = np.load(path_to_data, encoding="bytes")["latents_values"][
            ::subsample
        ]
    ...
    def __getitem__(self, idx):
        ...
        if self.image:
            ...
            return (
                image.float(),
                torch.from_numpy(latent_values).unsqueeze(1).float(),
                torch.from_numpy(label).unsqueeze(1).float(),
            )
...
def get_dsprites_dataloader(
    ...
    dsprites_data = DSpritesDataset(
        path_to_data, transform=transforms.ToTensor(), subsample=subsample, image=image
    )
    ...
    train_loader = DataLoader(
        dsprites_data, batch_size=batch_size, sampler=train_sampler, drop_last=True
    )
    validation_loader = DataLoader(
        dsprites_data, batch_size=batch_size, sampler=valid_sampler, drop_last=True
    )
